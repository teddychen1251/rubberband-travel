...
export class Avatar {
    ...
    constructor(scene: Scene, fpView: FreeCamera) {
        this.scene = scene;
        this.fpView = fpView;
        this.body = MeshBuilder.CreateSphere("avatar", { diameter: 0.05 }, this.scene);
        this.body.position.y += 0.0001 + 0.025;
        this.body.physicsImpostor = new PhysicsImpostor(this.body, PhysicsImpostor.BoxImpostor, {
            mass: 68, restitution: 0.2, friction: .3
        }, this.scene);
        this.body.isVisible = false;
        this.head = new TransformNode("head", this.scene);
        scene.registerAfterRender(() => {
            this.head.position = this.body.position.add(new Vector3(0, Avatar.HEIGHT, 0));
            if (!this.nearGround()) {
    ...
    nearGround() : boolean {
        return this.body.position.y - .025 <= 0.02;
    }
}